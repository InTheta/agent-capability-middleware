#!/usr/bin/env node
import { createDeveloperServiceOffer, createUserCapabilityOffer, LocalCapabilityDirectory, listOmniAgentRecipes, requireFreshPaidResult, runDesignPartnerCheck, searchCdpX402Bazaar, } from "./index.js";
...
if (command === "help" || command === "--help" || command === "-h") {
    ...
    try {
        ...
        const resource = result.resources.find((candidate) => candidate.resource.includes("/market-risk/"))
            ?? result.resources[0];
        if (!resource)
            throw new Error("No matching x402 resource was returned by CDP Bazaar");
...
function runDemo(flow) {
    ...
    if (flow !== "exchange")
        throw new TypeError(`Unknown demo flow: ${flow}`);
    printDemo(flow, {
...
function printDemo(flow, result) {

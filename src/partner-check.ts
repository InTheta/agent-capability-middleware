...
export class DesignPartnerCheckError extends Error {
  constructor(readonly step: "catalog" | "gateway" | "payment" | "revocation", message: string) {
    super(message);
    this.name = "DesignPartnerCheckError";
  }
}

export async function runDesignPartnerCheck(
  options: DesignPartnerCheckOptions = {},
): Promise<DesignPartnerCheckReport> {
  const started = new Date();
  const fetchImplementation = options.fetch ?? globalThis.fetch;
  const receiver = options.receiver ?? CANONICAL_OMNI_RECEIVER;
  let resources: BazaarResource[];
  try {
    resources = (await listCdpX402MerchantResources(
      receiver,
      { limit: 100 },
      fetchImplementation,
    )).resources;
  ...
  for (const route of CANONICAL_OMNI_ROUTES) {
    if (!listedRouteSet.has(route)) throw new DesignPartnerCheckError("catalog", `Canonical Omni route is not listed in CDP Bazaar: ${route}`);
  }
  if (!listing) throw new DesignPartnerCheckError("catalog", "Canonical Omni market-risk route is not listed in CDP Bazaar");
  const quote = listing.accepts.find((accept) => accept.network === "eip155:84532");
  if (!quote) throw new DesignPartnerCheckError("catalog", "Canonical route has no Base Sepolia quote");
  if (quote.amount !== "10000") throw new DesignPartnerCheckError("catalog", `Expected 10000 atomic USDC; received ${quote.amount}`);
  ...
  const catalog: DesignPartnerCheckReport["catalog"] = {
    source: "cdp_bazaar",
  ...
  let body;
  try {
    paid = await client.consumeX402Testnet<{
      schema: string;
  ...
  try {
    ...
    if (!revoked.revokedAt) throw new Error("Gateway did not confirm grant revocation");
  ...
  return finish(started, {
    ...
    payment: {
      ...
      auditEventId,
      schema: body.schema as "market_risk_snapshot.v1",
      freshness: "fresh",
    },
    revocation: {
      grantId: grant.id,
...
function finish(
  started: Date,
  report: Omit<DesignPartnerCheckReport, "startedAt" | "completedAt" | "elapsedMs">,
): DesignPartnerCheckReport {
